...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!reference || !reference.startsWith("krl::")) {
      console.warn(`[Wompi Webhook] Ignoring event with invalid or missing reference: ${reference}`);
      return NextResponse.json({ status: "ignored", reason: "Invalid Reference" });
    }
    ...
    if (!sessionId || !sourceNodeId) {
       console.warn(`[Wompi Webhook] Could not parse session info from reference: ${reference}`);
       return NextResponse.json({ status: "ignored", reason: "Malformed Reference" });
    }

    const session = await db.chatSession.findUnique({ where: { id: sessionId }, select: { chatAgent: true } });
    if (!session) {
        console.error(`[Wompi Webhook] Session with ID ${sessionId} not found.`);
        return NextResponse.json({ status: "error", reason: "Session Not Found" }, { status: 404 });
    }
    ...
    if (!wompiIntegration || !wompiIntegration.eventsSecret) {
      console.error(`[Wompi Webhook] Wompi integration or eventsSecret not found for user ${session.chatAgent.userId}`);
      return NextResponse.json({ status: "error", reason: "Integration Not Configured" }, { status: 400 });
    }

    // 4. Verificar la firma (checksum).
    const signatureProperties = payload.signature.properties;
    const stringToSign = signatureProperties
    ...
    if (payload.event === "transaction.updated" && payload.data.transaction.status === "APPROVED") {
      console.log(`[Wompi Webhook] Approved transaction ${payload.data.transaction.id}. Enqueuing task for worker...`);
      ...
      await redisClient.xadd(STREAM_KEY, "*", "payload", JSON.stringify(workerPayload));

      console.log("[Wompi Webhook] Task enqueued successfully.");
    } else {
        console.log(`[Wompi Webhook] Ignoring event type '${payload.event}' with status '${payload.data.transaction.status}'.`);
    }
