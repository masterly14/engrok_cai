...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const { name, company, email, phone, stageId, lastContact, status, tags, notes, value } = body;

        if (!name || !company || !email || !phone || !stageId || !lastContact) {
            return NextResponse.json({ error: 'Missing required fields: name, company, email, phone, stageId, lastContact are required.' }, { status: 400 });
        }
