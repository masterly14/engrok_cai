  ...
  try {
    ...
    if (!assistantId) {
      return NextResponse.json(
        { error: "assistantId missing in payload" },
        { status: 400 },
      );
    }
    ...
    const results = await Promise.all(
      toolCalls.map(async (call) => {
        ...
        try {
          ...
          if (!tool.provider) {
            throw new Error(
              `Tool "${name}" does not have a configured provider.`,
            );
          }
          ...
          if (!connection) {
            throw new Error(
              `No connection found for provider "${tool.provider}" for this user.`,
            );
          }
          ...
          if (name === "getAvailability") {
            ...
            const list = (data.availability || []).slice(0, 5) as {
              start: string;
            }[];
            result =
              list.length > 0
