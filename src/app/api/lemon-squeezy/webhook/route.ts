...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.log("[LS Webhook] Parsed payload:", JSON.stringify(payload, null, 2));
  } catch (err) {
  ...
  console.log(`[LS Webhook] Acknowledging receipt and starting async processing for event: ${payload.meta?.event_name}`);
...
async function handleEvent(payload: any) {
  ...
  switch (type) {
    case "subscription_created":
      console.log('[LS Webhook] Event: subscription_created. Payload:', JSON.stringify(payload.data.attributes, null, 2));
      await upsertSubscription(payload);
      await syncInternalPlanAndSubscription(payload, /* isNew */ true);
      break;
    case "subscription_updated":
      console.log('[LS Webhook] Event: subscription_updated. Payload:', JSON.stringify(payload.data.attributes, null, 2));
      await upsertSubscription(payload);
      await syncInternalPlanAndSubscription(payload);
      break;
    case "subscription_cancelled":
      console.log('[LS Webhook] Event: subscription_cancelled. Payload:', JSON.stringify(payload.data.attributes, null, 2));
      await upsertSubscription(payload);
      await markSubscriptionCancelled(payload);
      break;
    case "subscription_payment_success":
    case "invoice_payment_succeeded":
      console.log(`[LS Webhook] Event: ${type}. Payload:`, JSON.stringify(payload.data.attributes, null, 2));
      ...
      break;
    case "order_created":
      console.log('[LS Webhook] Event: order_created. Payload:', JSON.stringify(payload.data.attributes, null, 2));
...
async function upsertSubscription(payload: any) {
  ...
  if (!userId) {
    console.warn("[LS Webhook] upsertSubscription: User not found for LS subscription", sub.id);
    return;
  }
  ...
  if (!planVariantId) {
    console.error("[LS Webhook] upsertSubscription: Could not determine variantId. Cannot upsert subscription.");
    return;
  }
  console.log(`[LS Webhook] upsertSubscription: Determined variantId: ${planVariantId}`);
  ...
  console.log(`[LS Webhook] upsertSubscription: Initial plan details - Price: ${price}, Variant Name: ${variantName}, Product ID: ${productId}`);
  ...
  if (!lsPlan) {
    console.log(`[LS Webhook] upsertSubscription: LsPlan with variantId ${planVariantId} not found. Creating it...`);

    // Si no tenemos precio todavía, lo consultamos vía API
    if (price === undefined) {
      try {
        console.log(`[LS Webhook] upsertSubscription: Price not in webhook payload for variant ${planVariantId}. Fetching from API...`);
        const variantData: any = await lemonSqueezy.getVariant(planVariantId);
        price = variantData.data.attributes.price;
        console.log(`[LS Webhook] upsertSubscription: Fetched price from API: ${price}`);
      } catch (error) {
        console.error(`[LS Webhook] upsertSubscription: Error fetching variant ${planVariantId} from Lemon Squeezy API:`, error);
        return;
      }
    }

    if (price === undefined) {
      console.error(`[LS Webhook] upsertSubscription: Could not determine price for variant ${planVariantId}. Cannot create plan.`);
      return;
    }

    if (productId === undefined) {
      console.error("[LS Webhook] upsertSubscription: Cannot create plan without productId");
      return;
    }

    console.log(`[LS Webhook] upsertSubscription: Creating new lsPlan with data:`, { variantId: planVariantId, productId: Number(productId), storeId: storeId, name: variantName, price: price });
    ...
    console.log(`[LS Webhook] upsertSubscription: Created new lsPlan "${variantName}" (ID: ${lsPlan.id}) from webhook.`);
  } else {
    // Cuando el plan ya existe, usamos su información como fallback
    console.log(`[LS Webhook] upsertSubscription: Found existing lsPlan with ID: ${lsPlan.id}`);
    price = price ?? lsPlan.price;
    productId = productId ?? lsPlan.productId;
    variantName = variantName ?? lsPlan.name;
    console.log(`[LS Webhook] upsertSubscription: Using effective plan details - Price: ${price}, Product ID: ${productId}, Variant Name: ${variantName}`);
  }

  // ---------------- Plan de negocio interno ----------------
  const priceUsd = (price ?? 0) / 100;
  const creditsPerCycleCalc = computeIncludedCredits(planVariantId, priceUsd);
  console.log(`[LS Webhook] upsertSubscription: Internal plan details - Price USD: ${priceUsd}, Calculated Credits/Cycle: ${creditsPerCycleCalc}`);
  ...
  if (!plan) {
    ...
    console.log("[LS Webhook] upsertSubscription: Internal plan created:", plan);
  } else if (plan.creditsPerCycle !== creditsPerCycleCalc) {
    console.log(`[LS Webhook] upsertSubscription: Internal plan credits mismatch (${plan.creditsPerCycle} vs ${creditsPerCycleCalc}). Updating...`);
    ...
    console.log("[LS Webhook] upsertSubscription: Internal plan updated:", plan);
  } else {
    console.log("[LS Webhook] upsertSubscription: Found existing internal plan:", plan);
  }

  if (!plan) {
    console.error("[LS Webhook] upsertSubscription: Failed to create or fetch internal Plan");
    return;
  }
  ...
  console.log("[LS Webhook] upsertSubscription: Upserted LsSubscription result:", subscription);
  // Sincronizaremos créditos en la fase `syncInternalPlanAndSubscription`, donde ya existe la suscripción interna.
  const user = await db.user.findUnique({ where: { id: userId } });
  console.log('[LS Webhook] upsertSubscription: Checking user for initial credit grant:', user);
  if (user && user.initialAmountCredits === 0) {
    console.log(`[LS Webhook] upsertSubscription: User has 0 initial credits. Granting ${plan.creditsPerCycle} initial credits.`);
...
async function resolveUserId(payload: any): Promise<string | null> {
  ...
  try {
    ...
    if (uid && typeof uid === "string") {
      console.log(`[LS Webhook] resolveUserId: Found potential user ID in custom data: ${uid}`);
      const userById = await db.user.findUnique({ where: { id: uid } });
      console.log('[LS Webhook] resolveUserId: User lookup by ID result:', userById ? `found user ${userById.id}` : 'not found');
    ...
    if (email) {
      ...
      console.log('[LS Webhook] resolveUserId: User lookup by email result:', userByEmail ? `found user ${userByEmail.id}` : 'not found');
...
async function syncInternalPlanAndSubscription(payload: any, isNew: boolean = false) {
  console.log(`[LS Webhook] syncInternalPlanAndSubscription: Starting... isNew=${isNew}`);
  const sub = payload.data;
  const attributes = sub.attributes as any;

  const userId = await resolveUserId(payload);
  if (!userId) {
    console.log("[LS Webhook] syncInternalPlanAndSubscription: Could not resolve user ID. Aborting.");
    return;
  }
  console.log(`[LS Webhook] syncInternalPlanAndSubscription: Resolved userId: ${userId}`);

  // 1) Plan (mapeado por variantId)
  const variantId = attributes.variant_id ?? attributes.first_order_item?.variant_id;
  if (!variantId) {
    console.log("[LS Webhook] syncInternalPlanAndSubscription: No variantId found. Aborting.");
    return;
  }
  ...
  console.log(`[LS Webhook] syncInternalPlanAndSubscription: Calculated approxCredits: ${approxCredits}`);

  // ----- Internal Plan (biz) -----
  let plan = await db.plan.findUnique({ where: { variantId } });
  console.log('[LS Webhook] syncInternalPlanAndSubscription: Internal plan search result: ', plan ? `found plan ${plan.id}` : 'not found');
  if (!plan) {
    console.log('[LS Webhook] syncInternalPlanAndSubscription: Internal plan not found, creating...');
    if (productId === undefined) {
      console.log("[LS Webhook] syncInternalPlanAndSubscription: Cannot create plan without productId. Aborting.");
      return;
    }
    ...
    console.log('[LS Webhook] syncInternalPlanAndSubscription: Plan created: ', plan);
  } else if (plan.creditsPerCycle !== approxCredits && approxCredits > 0) {
    console.log(`[LS Webhook] syncInternalPlanAndSubscription: Plan credits mismatch (${plan.creditsPerCycle} vs ${approxCredits}). Updating...`);
  ...
  if (!plan) {
    console.log("[LS Webhook] syncInternalPlanAndSubscription: Plan could not be found or created. Aborting.");
    return;
  } // safety

  // --- Garantizar créditos por plan ---
  console.log(`[LS Webhook] syncInternalPlanAndSubscription: Checking for credit guarantee. Current creditsPerCycle: ${plan.creditsPerCycle}`);
  if (plan.creditsPerCycle === 0) {
    ...
    console.log(`[LS Webhook] syncInternalPlanAndSubscription: Mapped credits by name: ${mapped}`);

    if (mapped > 0) {
      console.log(`[LS Webhook] syncInternalPlanAndSubscription: Updating plan with guaranteed credits: ${mapped}`);
  ...
  console.log(`[LS Webhook] syncInternalPlanAndSubscription: Searching for internal subscription with LS ID: ${sub.id.toString()}`);
  let subscription = await db.subscription.findFirst({
    where: { lemonSqueezyId: sub.id.toString() },
  });
  console.log(`[LS Webhook] syncInternalPlanAndSubscription: Internal subscription search result:`, subscription ? `found sub ${subscription.id}` : 'not found');
  ...
  if (!subscription) {
    ...
    console.log("[LS Webhook] syncInternalPlanAndSubscription: Internal subscription created:", subscription);

    // Otorgar créditos iniciales
    if (plan.creditsPerCycle > 0) {
      console.log(`[LS Webhook] syncInternalPlanAndSubscription: New subscription. Granting ${plan.creditsPerCycle} initial credits.`);
      await CreditService.credit(userId, plan.creditsPerCycle, {
        reason: "subscription_created",
        subId: subscription.id,
      });
      console.log(`[LS Webhook] syncInternalPlanAndSubscription: Initial credits granted.`);
    }
  } else {
    ...
    console.log("[LS Webhook] syncInternalPlanAndSubscription: Internal subscription updated.");

    // Si aún no tiene créditos, otorgar los incluidos en el plan
    console.log(`[LS Webhook] syncInternalPlanAndSubscription: Checking if existing subscription needs credits. Current credits: ${subscription.currentCredits}, Plan credits: ${plan.creditsPerCycle}`);
    if (subscription.currentCredits === 0 && plan.creditsPerCycle > 0) {
      console.log(`[LS Webhook] syncInternalPlanAndSubscription: Granting ${plan.creditsPerCycle} credits on sync.`);
      await CreditService.credit(userId, plan.creditsPerCycle, {
        reason: "subscription_sync",
        subId: subscription.id,
      });
      console.log(`[LS Webhook] syncInternalPlanAndSubscription: Credits granted on sync.`);
    }
  }
}
...
async function markSubscriptionCancelled(payload: any) {
  ...
  const sub = payload.data;
  const attributes = sub.attributes as any;
  console.log(`[LS Webhook] markSubscriptionCancelled: Looking for internal subscription with LS ID: ${sub.id.toString()}`);
  const internal = await db.subscription.findFirst({ where: { lemonSqueezyId: sub.id.toString() } });
  if (internal) {
    console.log(`[LS Webhook] markSubscriptionCancelled: Found internal subscription ${internal.id}. Updating status to CANCELLED.`);
    await db.subscription.update({
    ...
    console.log("[LS Webhook] markSubscriptionCancelled: No internal subscription found.");
  }
}
...
async function creditOnSuccessfulPayment(payload: any) {
  ...
  const sub = payload.data;
  const attributes = sub.attributes as any;
  console.log(`[LS Webhook] creditOnSuccessfulPayment: Looking for internal subscription with LS ID: ${sub.id.toString()}`);
  ...
  if (!internal || !internal.plan) {
    console.log("[LS Webhook] creditOnSuccessfulPayment: Internal subscription or plan not found. Aborting. Sub:", internal);
    return;
  }
  console.log(`[LS Webhook] creditOnSuccessfulPayment: Found internal sub ${internal.id} with plan ${internal.plan.id}.`);

  // Reponer créditos según plan
  if (internal.plan.creditsPerCycle > 0) {
    console.log(`[LS Webhook] creditOnSuccessfulPayment: Crediting user ${internal.userId} with ${internal.plan.creditsPerCycle} credits for payment.`);
    await CreditService.credit(internal.userId, internal.plan.creditsPerCycle, {
      reason: "cycle_payment",
      subId: internal.id,
    });
    console.log(`[LS Webhook] creditOnSuccessfulPayment: Credits granted. Updating subscription cycle date.`);
    ...
    console.log(`[LS Webhook] creditOnSuccessfulPayment: Subscription cycle updated to ${nextCycle}.`);
  } else {
    console.log("[LS Webhook] creditOnSuccessfulPayment: Plan has no credits to cycle. Skipping credit grant.");
  }
}
...
async function processCreditPackOrder(payload: any) {
  ...
  if (!variantId) {
    console.log("[LS Webhook] processCreditPackOrder: No variantId in order. Aborting.");
    return;
  }
  console.log(`[LS Webhook] processCreditPackOrder: Order has variantId: ${variantId}`);

  const credits = CREDIT_PACKS[variantId];
  if (!credits) {
    console.log(`[LS Webhook] processCreditPackOrder: VariantId ${variantId} is not a credit pack. Skipping.`);
    return;
  }
  console.log(`[LS Webhook] processCreditPackOrder: It's a credit pack for ${credits} credits.`);

  const email = attributes.user_email;
  const userId = await resolveUserId(payload);
  if (!userId) {
    console.log(`[LS Webhook] processCreditPackOrder: Could not resolve user for email ${email}. Aborting.`);
    return;
  }
  console.log(`[LS Webhook] processCreditPackOrder: Resolved userId: ${userId}. Granting credits.`);
  ...
  console.log("[LS Webhook] processCreditPackOrder: Credits granted successfully.");
