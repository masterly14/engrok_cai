...
export function ConversationNodeConfig({
  ...
  const { assetsData, assetsLoading, assetsError, refetchAssets } =
    useAssets(true);
  ...
  useEffect(() => {
    if (isLimitedToTemplate && data.responseType !== "template") {
      ...
      toast.warning(
        "El primer nodo que acepta cualquier mensaje ha sido configurado como plantilla.",
      );
    }
  ...
  const handleResponseTypeChange = (value: ResponseType) => {
    ...
    if (isLimitedToTemplate && value !== "template") {
      toast.error(
        "El primer nodo que acepta cualquier mensaje debe ser un mensaje plantilla.",
      );
      return;
    }
  ...
  const submitAudio = async () => {
    if (!audioBlob) {
      toast.error(
        "No hay audio para subir. Graba o selecciona un archivo primero.",
      );
      return;
    }
  ...
  const handleMediaUpload = async (
    ...
    if (file.type.startsWith("video/") && file.size > 16 * 1024 * 1024) {
      toast.error(
        "El archivo de video no puede superar los 16 MB. Por favor, selecciona un archivo más pequeño.",
      );
      return;
    }
    ...
    try {
      ...
      if (isLargeFile) {
        ...
        const { signature, apiKey, cloudName, error } =
        ...
        if (result.secure_url) {
          ...
          toast.error(
            `Error subiendo archivo: ${result.error?.message || "Desconocido"}`,
          );
        }
      } else {
        const formData = new FormData();
        ...
        if (result.success && result.url) {
          ...
          toast.error(
            `Error al subir archivo: ${result.error || "Desconocido"}`,
          );
        }
      }
  ...
  const handleDeleteMedia = async () => {
    ...
    setIsUploadingMedia(true);
    try {
      const delRes = await deleteCloudinaryFile(
        data.filePublicId,
        data.fileResourceType || "raw",
      );
      if (!delRes.success) {
  ...
  const hasButtons =
    Array.isArray(data.interactiveButtons) &&
  ...
  const { templatesData, templatesLoading, templatesError } =
    useMessageTemplates(agentId);
  ...
  const templatePlaceholders = useMemo(
    () => extractPlaceholders(data.templateBody),
    [data.templateBody],
  );

  // Sync: si aparecen nuevos placeholders, asegura que existan en templateVariableValues
  useEffect(() => {
    if (responseType !== "template" || templatePlaceholders.length === 0)
      return;
  ...
  const hasUnfilledTemplateVars = useMemo(() => {
    ...
    return templatePlaceholders.some(
      (ph) => !(data.templateVariableValues?.[ph] ?? "").trim(),
    );
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">
              Configurar nodo de conversación
            </h1>
      ...
      {isLimitedToTemplate && (
        <Alert className="border-orange-200 bg-orange-50">
          ...
          <AlertDescription className="text-orange-800">
            Este es el primer nodo de conversación del flujo y acepta cualquier
            mensaje, por lo que debe ser un mensaje plantilla.
          </AlertDescription>
        </Alert>
      )}
            ...
            <div className="flex items-center space-x-3 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Define el nombre y comportamiento del nodo
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <Input
                  id="node-name"
                  value={
                    responseType === "template" ? data.templateName : data.name
                  }
              ...
              <div>
                ...
                <RadioGroup
                  ...
                  onValueChange={(val: string) =>
                    handleResponseTypeChange(val as ResponseType)
                  }
                ...
                >
                  <div
                    ...
                    onClick={() =>
                      !isLimitedToTemplate && handleResponseTypeChange("text")
                    }
                  >
                    <RadioGroupItem
                      value="text"
                      id="r-text"
                      disabled={isLimitedToTemplate}
                    />
                  ...
                  <div
                    ...
                    onClick={() =>
                      !isLimitedToTemplate && handleResponseTypeChange("audio")
                    }
                  >
                    <RadioGroupItem
                      value="audio"
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Envía un mensaje de audio o un archivo de audio
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 border rounded-lg">
                    <div>
                      <Label className="font-medium">
                        Aceptar cualquier mensaje
                      </Label>
                      <p className="text-sm text-gray-500 mt-1">
                        Este nodo se activará con cualquier mensaje del usuario
                        si no hay otro nodo que coincida
                      </p>
                    </div>
                    <Switch
                      checked={data.acceptAnyMessage || false}
                      onCheckedChange={(checked) =>
                        handleChange("acceptAnyMessage", checked)
                      }
                    />
                  </div>
                  ...
                  {!data.acceptAnyMessage && (
                    <div>
                      ...
                      <Textarea
                        ...
                        onChange={(e) =>
                          handleChange("userResponse", e.target.value)
                        }
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        Palabras o frases que activarán este nodo
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div>
                ...
                <div className="flex items-center justify-between p-4 border rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      Saltar automáticamente al siguiente nodo después de enviar
                      la respuesta
                    </p>
                  </div>
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      handleChange("jumpToNextNode", checked)
                    }
            ...
            <div className="bg-white rounded-lg border p-6">
              <div className="flex items-center space-x-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Configura el texto que enviará el bot
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    El mensaje que será enviado al usuario
                  </p>
                </div>
            ...
            <div className="bg-white rounded-lg border p-6">
              <div className="flex items-center space-x-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Graba o sube un mensaje de audio
                  </p>
                </div>
              </div>
            ...
            <div className="bg-white rounded-lg border p-6">
              <div className="flex items-center space-x-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Selecciona y configura una plantilla de mensaje
                  </p>
                </div>
              </div>
                    ...
                    onValueChange={(value) => {
                      ...
                      if (tpl) {
                        ...
                        let extractedButtons: { id: string; title: string }[] =
                          [];
                        const btnComp = components?.find(
                          (c: any) => c.type === "BUTTONS",
                        );
                        if (
                          btnComp?.buttons &&
                          Array.isArray(btnComp.buttons)
                        ) {
                        ...
                        if (tpl.category === "AUTHENTICATION") {
                          const otpBtn = btnComp?.buttons?.find(
                            (b: any) => b.type === "OTP",
                          );
                  ...
                  (Array.isArray(templatesData?.data)
                    ...
                    <Alert className="border-yellow-200 bg-yellow-50">
                      ...
                      <AlertDescription className="text-yellow-800">
                        Aún no tienes plantillas de mensaje creadas. Debes crear
                        una antes de poder usar este flujo.
                        <Button asChild className="mt-2 ml-2" size="sm">
                          <Link href="/application/agents/chat-agents/templates">
                            Crear Plantilla
                          </Link>
                        </Button>
                      </AlertDescription>
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {templatePlaceholders.map((ph) => {
                        const currentVal =
                          data.templateVariableValues?.[ph] || "";
                        const isVar = isVariableVal(currentVal);
                        const selectedOption = isVar
                          ? stripBraces(currentVal)
                          : "__custom__";
                          ...
                          <div key={ph} className="border rounded-lg p-4">
                            ...
                            {selectedOption === "__custom__" && (
                              <Input
                                value={currentVal}
                                onChange={(e) =>
                                  handleTemplateVarChange(ph, e.target.value)
                                }
                      ...
                      {hasUnfilledTemplateVars && (
                        <Alert className="border-red-200 bg-red-50">
                          ...
                          <AlertDescription className="text-red-800">
                            Debes asignar un valor para todas las variables
                            antes de guardar el flujo.
                          </AlertDescription>
                        </Alert>
                      )}
                    </div>
            ...
            {data.jumpToNextNode ? (
              <div className="text-center py-12 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">
                  Botones interactivos desactivados
                </h3>
                <p className="text-sm text-gray-600 mb-4">
                  Desactiva "Saltar al siguiente nodo" en la configuración
                  básica para habilitarlos
                </p>
                <Button variant="outline" onClick={() => setCurrentStep(1)}>
              ...
              <InteractiveButtonsConfig
                ...
                onChange={(buttons: any) =>
                  handleChange("interactiveButtons", buttons)
                }
              />
            )}
          </div>
