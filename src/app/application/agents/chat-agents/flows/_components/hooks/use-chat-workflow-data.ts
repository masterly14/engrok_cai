          ...
          data = {
            ...
            body:
              ...
              JSON.stringify({
                ...
                name: minNode.name || "New Contact", // This might be redundant if 'name' is top-level
                stage: minNode.stage || "",
                phone: minNode.phone || "",
                tag: minNode.tag || "",
                notes: minNode.notes || "",
  ...
  const builderEdges: Edge[] = minEdges
    .map((minEdge: any, idx: number) => {
      const sourceNode = builderNodes.find(
        (n) => n.data.name === minEdge.from || n.id === minEdge.from
      ); // Match by name or ID
      const targetNode = builderNodes.find(
        (n) => n.data.name === minEdge.to || n.id === minEdge.to
      ); // Match by name or ID

      if (!sourceNode || !targetNode) {
        console.warn(
          `Edge references non-existent node: from '${minEdge.from}' to '${minEdge.to}'. Skipping edge.`
        );
        return null;
      }

      return {
        id: minEdge.id || `e-${sourceNode.id}-${targetNode.id}-${idx}`, // More robust edge ID
        source: sourceNode.id,
        target: targetNode.id,
      };
    })
    .filter((edge) => edge !== null) as Edge[]; // Filter out null edges

  return { builderNodes, builderEdges };
}

export function useChatWorkflowData({
  workflowId,
  setNodes,
  setEdges,
}: UseChatWorkflowDataProps) {
  const [workflowName, setWorkflowName] = useState("Nuevo Flujo");
  const [agentData, setAgentData] = useState<any>(null);
  const [savedWorkflowId, setSavedWorkflowId] = useState(workflowId);
  ...
  useEffect(() => {
    const loadWorkflow = async () => {
      if (!workflowId) {
        // New workflow - set default name and clear nodes/edges
        ...
        if (response.status === 200 && response.workflow) {
          ...
          if (workflow.workflow) {
            try {
              ...
              console.log("Raw data loaded from DB:", workflowData);
              console.log("Loaded workflow data:", workflowData);

              // Check if it's the ReactFlow format (has nodes with id, type, position, data)
              ...
              // Check if it's the minimal format (has nodes with type but no id)
              ...
              console.log("Is ReactFlow format:", isReactFlowFormat);
              console.log("Is minimal format:", isMinimalFormat);

              if (isReactFlowFormat) {
                // Use the data as-is (ReactFlow format)
                const { nodes: loadedNodes = [], edges: loadedEdges = [] } =
                  workflowData;
                setNodes(loadedNodes);
                setEdges(loadedEdges);
                console.log("Loaded ReactFlow format nodes:", loadedNodes);
                console.log("Loaded ReactFlow format edges:", loadedEdges);
              } else if (isMinimalFormat) {
                // Transform from minimal format
                const { builderNodes, builderEdges } =
                  transformFromMinimalFormat(
                    workflowData.nodes,
                    workflowData.edges || [] // Ensure edges is an array
                  );
                setNodes(builderNodes);
                setEdges(builderEdges);
                console.log("Transformed minimal format nodes:", builderNodes);
                console.log("Transformed minimal format edges:", builderEdges);
              } else {
                ...
                setNodes(loadedNodes);
                setEdges(loadedEdges);
                console.log("Fallback loaded nodes:", loadedNodes);
                console.log("Fallback loaded edges:", loadedEdges);
              }
            } catch (error) {
              console.error(
                "Error parsing or transforming workflow data:",
                error
              );
              toast.error(
                "Error parseando o transformando los datos del flujo"
              );
            }
          } else {
            setNodes([]);
          ...
          console.warn(
            "Workflow not found or error fetching for ID:",
            workflowId
          );
          setNodes([]);
          setEdges([]);
