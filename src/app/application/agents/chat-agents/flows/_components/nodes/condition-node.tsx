...
const ConditionNode = ({
  data,
}: NodeProps<{ name?: string; condition?: string; statusSuccess?: string; statusError?: string }>) => {
  return (
    <Card className="w-72 shadow-lg border-gray-200/80 bg-white">
      <CardHeader className="p-4 flex flex-row items-center gap-3 space-y-0 bg-gray-50/70 rounded-t-lg">
        ...
        <div>
          <CardTitle className="text-sm font-semibold text-gray-900">{data.name || "Condition"}</CardTitle>
        </div>
      </CardHeader>

      <CardContent className="p-4 text-sm text-gray-700">
        <p className="line-clamp-3">{data.condition || "Configure la condición..."}</p>
      </CardContent>
      ...
      <div className="border-t border-gray-200 px-4 py-2 space-y-2 bg-gray-50/50 rounded-b-lg">
        <p className="text-xs font-medium text-gray-500 mb-2">Salidas Condicionales:</p>
