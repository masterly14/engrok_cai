...
const validateConnection = (sourceNode: Node, targetNode: Node, allNodes: Node[]): boolean => {
  if (sourceNode.type === "trigger") {
    ...
    if (!targetNode.data.initialMessage) {
      toast.error("El nodo Trigger solo puede conectarse al primer nodo de Conversación del flujo.")
      return false
    }
  }
  if (targetNode.data.initialMessage && targetNode.type === "conversation") {
    if (sourceNode.type !== "trigger") {
      toast.error("El primer nodo de Conversación solo puede recibir conexiones desde un nodo Trigger.")
      return false
    }
  }
  if (sourceNode.type === "captureResponse" && targetNode.type === "captureResponse") {
    toast.error("No puedes conectar dos nodos de captura de respuesta entre sí")
    return false
  }
  ...
  const onConnect = useCallback(
    (params: Connection) => {
      ...
      if (sourceNode.type === "conversation" && params.sourceHandle) {
        ...
        const matchedBtn = buttons.find((b: any) => b.id === params.sourceHandle)
        if (matchedBtn) {
  ...
  return (
    <div className="w-full h-full flex-1 relative bg-gradient-to-br from-gray-50 to-gray-100" ref={reactFlowWrapper}>
      <ReactFlowProvider>
        ...
        >
          <Background gap={24} size={1} color="#94a3b8" variant={BackgroundVariant.Dots} />
                    ...
                    <div className="flex items-center gap-1">
                      {isEditingName ? (
                        <>
                          ...
                          />
                          <button className="p-1 hover:bg-emerald-100 rounded" onClick={handleRenameSave}>
                        ...
                        <>
                          ...
                          <button className="p-1 hover:bg-gray-100 rounded" onClick={() => setIsEditingName(true)}>
                            <Pencil className="w-3 h-3 text-gray-500" />
                          </button>
                        </>
                      )}
                    </div>
                    <p className="text-xs text-gray-500">Diseña tu flujo de conversación</p>
                  </div>
                </div>
                <div className="h-6 w-px bg-gray-200" />
                <div className="flex items-center gap-2">
                  <Button variant="ghost" size="sm" className="h-8 text-xs" onClick={handleSave} disabled={isPending}>
                    {isPending ? (
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <div className="absolute top-0 left-0 w-full h-screen bg-black/50 z-50">
              <NodeSelector onSelect={addNode} onClose={() => setIsMenuOpen(false)} />
            </div>
          )}
        </AnimatePresence>
        ...
        <Dialog open={isTestDialogOpen} onOpenChange={setIsTestDialogOpen}>
          <DialogContent className="sm:max-w-md">
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setIsTestDialogOpen(false)} disabled={isTesting}>
                Cancelar
              </Button>
              <Button onClick={() => sendTestMessage(testPhone)} disabled={isTesting || testPhone.trim() === ""}>
                {isTesting ? <Loader2 className="h-4 w-4 mr-1.5 animate-spin" /> : <Send className="h-4 w-4 mr-1.5" />}
                Enviar
              </Button>
            </DialogFooter>
          </DialogContent>
        ...
        <Dialog open={isConditionModalOpen} onOpenChange={setIsConditionModalOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Describe la condición que debe cumplirse para seguir esta ruta desde el nodo de IA.
              </DialogDescription>
            </DialogHeader>
            <div className="py-4">
              ...
              <Input
                ...
                placeholder={conditionSuggestion || "Ej: Si el usuario pide hablar con un humano"}
                className="mt-2"
              />
              <p className="text-xs text-gray-500 mt-2">Sugerencia: {conditionSuggestion}</p>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setIsConditionModalOpen(false)}>
                Cancelar
              </Button>
              <Button onClick={handleConfirmCondition}>Confirmar Conexión</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </ReactFlowProvider>
