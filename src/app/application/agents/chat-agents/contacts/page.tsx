...
const AddContactForm = ({closeModal}: {closeModal: () => void}) => {
  ...
  const mutation = useMutation({
    mutationFn: (variables: { contacts: { phone: string; name?: string }[], chatAgentId: string }) => 
          ...
          <div className="space-y-4">
            {!file ? (
              <div className="flex items-center justify-center w-full">
                <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-lg cursor-pointer bg-slate-50 hover:bg-slate-100 border-slate-300">
              ...
              <div className="space-y-4">
                ...
                <p className="text-sm text-slate-600">Selecciona las columnas que corresponden al nombre y teléfono.</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="name-column" className="block text-sm font-medium text-slate-700 mb-1">Columna para el Nombre</label>
                    <Select onValueChange={setNameColumn} value={nameColumn}>
                      <SelectTrigger id="name-column"><SelectValue placeholder="Selecciona una columna" /></SelectTrigger>
                      <SelectContent>{headers.map((h) => <SelectItem key={h} value={h}>{h}</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
                  <div>
                    <label htmlFor="phone-column" className="block text-sm font-medium text-slate-700 mb-1">Columna para el Teléfono (Requerido)</label>
                    <Select onValueChange={setPhoneColumn} value={phoneColumn}>
                      <SelectTrigger id="phone-column"><SelectValue placeholder="Selecciona una columna" /></SelectTrigger>
                      <SelectContent>{headers.map((h) => <SelectItem key={h} value={h}>{h}</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
                </div>
            ...
            <div className="flex justify-end space-x-2 pt-4">
                ...
                <Button onClick={handleContinue} disabled={!file || !phoneColumn}>Continuar</Button>
            </div>
            {error && <p className="text-red-500 text-sm mt-2">{error}</p>}
          </div>
        )
      case 2: // Preview and Agent Selection
        const previewContacts = parsedData.slice(0, 5).map(row => ({ phone: row[phoneColumn], name: nameColumn ? row[nameColumn] : 'N/A' }));
        return (
          <div className="space-y-4">
            <h3 className="font-medium text-lg">Asignar Agente y Previsualizar</h3>
            <div>
              <label htmlFor="agent-select" className="block text-sm font-medium text-slate-700 mb-1">Asignar contactos al agente:</label>
              <Select onValueChange={setSelectedAgent} value={selectedAgent}>
                <SelectTrigger id="agent-select"><SelectValue placeholder="Selecciona un agente" /></SelectTrigger>
                <SelectContent>{chatAgents?.map(agent => <SelectItem key={agent.id} value={agent.id}>{agent.name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div>
                <h4 className="font-medium text-slate-800 mb-2">Vista Previa (primeras 5 filas)</h4>
                <div className="border rounded-md">
                    <Table>
                        <TableHeader><TableRow><TableHead>Nombre</TableHead><TableHead>Teléfono</TableHead></TableRow></TableHeader>
                        <TableBody>
                            {previewContacts.map((contact, i) => <TableRow key={i}><TableCell>{contact.name}</TableCell><TableCell>{contact.phone}</TableCell></TableRow>)}
                        </TableBody>
                    </Table>
                </div>
                <p className="text-sm text-slate-600 mt-2">Se importarán un total de <strong>{parsedData.length}</strong> contactos.</p>
            </div>
            <div className="flex justify-end space-x-2 pt-4">
              <Button variant="ghost" onClick={() => setStep(1)}>Atrás</Button>
              <Button onClick={handleImport} disabled={!selectedAgent}>Importar Contactos</Button>
            </div>
          </div>
        )
      case 3: // Loading
        return (
            <div className="flex flex-col items-center justify-center h-48">
                ...
                <h3 className="text-lg font-medium text-slate-700">Importando contactos...</h3>
        ...
        <div className="max-w-7xl mx-auto">
          <Card className="border-0 shadow-sm bg-red-50 border-red-200">
            <CardContent className="p-8 text-center">
              ...
              <h2 className="text-xl font-semibold text-red-800 mb-2">Error al cargar contactos</h2>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="space-y-1">
            ...
            <p className="text-slate-600">Gestiona todos tus contactos de WhatsApp</p>
          </div>
          ...
          <Card className="border-0 shadow-sm hover:shadow-md transition-shadow duration-200 bg-gradient-to-br from-green-50 to-green-100">
            <CardContent className="p-6">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <p className="text-sm font-medium text-green-700">Agentes Activos</p>
                  <p className="text-2xl font-bold text-green-900">{uniqueAgents}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-900">{recentContacts}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <CardContent>
                    ...
                    <TableRow>
                      <TableCell colSpan={4} className="text-center py-12">
                        <div className="flex flex-col items-center gap-4">
                          ...
                          <div>
                            <p className="text-slate-600 font-medium mb-1">No tienes contactos aún</p>
                            <p className="text-slate-500 text-sm mb-4">Comienza agregando tu primer contacto</p>
                            <Button
            ...
            {filteredContacts && filteredContacts.length > 0 && (
              <div className="mt-4 flex items-center justify-between text-sm text-slate-600">
                <p>
                  Mostrando {filteredContacts.length} de {totalContacts} contactos
                  {searchTerm && (
                    <span className="ml-1">
