...
export function Sidebar({ agents: initialAgents }: { agents: ChatAgentWithWorkflows[] }) {
  const { selectedChatAgent, setSelectedChatAgent, setIsCreatingNew } = useChatAgent()
        ...
        <div className="border-b backdrop-blur-sm">
          <div className="flex items-center gap-3 p-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">Gestiona tus chatbots</p>
            </div>
          </div>
              ...
              {isLoading ? (
                <div className="flex items-center justify-center py-12">
                  <div className="text-center">
                    <LoadingSpinner />
                    <p className="text-sm text-muted-foreground mt-3">Cargando agentes...</p>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-12 px-4">
                  ...
                  <p className="text-xs text-muted-foreground text-center mb-4">
                    {searchTerm ? "Intenta con otro término de búsqueda" : "Crea tu primer agente de chat para comenzar"}
                  </p>
                </div>
              ) : (
                filteredAgents.map((agent) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <Bot
                          className={cn(
                            ...
                            selectedChatAgent?.id === agent.id ? "text-white" : "text-muted-foreground",
                          )}
                        />
                      </div>
                      ...
                      {agent.isActive && (
                        <div className="h-2 w-2 rounded-full bg-green-400 shadow-sm" title="Activo" />
                      )}
                    </div>
                  </div>
                ))
