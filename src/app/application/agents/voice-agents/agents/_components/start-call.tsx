...
const StartCall = ({ vapiId }: Props) => {
  ...
  return (
    <>
      ...
      >
        <Phone className={`h-4 w-4 mr-2 ${activeCall ? "animate-pulse" : ""}`} />
        {isCalling
          ? activeCall
            ? "Cerrar llamada"
            : "Iniciar llamada"
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-left space-y-3">
              <p>
                <strong>Importante:</strong> Las llamadas de prueba también tienen un costo en créditos y no están
                exentas de cargos.
              </p>
              <p>Al continuar, confirmas que entiendes que esta llamada consumirá créditos de tu cuenta.</p>
              <p className="text-sm text-muted-foreground">Si aceptas esta confirmación no se te volverá a mostrar este mensaje.</p>
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="flex-col sm:flex-row gap-2">
            <Button variant="outline" onClick={() => setShowCostModal(false)} className="w-full sm:w-auto">
              Cancelar
            </Button>
            <Button onClick={handleAcceptCosts} className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700">
              Acepto y continuar
            </Button>
          </DialogFooter>
        </DialogContent>
