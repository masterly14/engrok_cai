...
export default function CallsTable({ llamadas, onSeleccionarLlamada }: CallsTableProps) {
  const [busqueda, setBusqueda] = useState("")
  const [filtroEstado, setFiltroEstado] = useState<string>("todos")
  const [ordenPor, setOrdenPor] = useState<"fecha" | "costo" | "duracion">("fecha")
  ...
  const llamadasFiltradas = llamadas.filter((llamada) => {
    ...
    const coincideEstado = filtroEstado === "todos" || llamada.status === filtroEstado
  ...
  const llamadasOrdenadas = [...llamadasFiltradas].sort((a, b) => {
    ...
    switch (ordenPor) {
      ...
      case "costo":
        ...
        break
      case "duracion":
        valorA = new Date(a.endedAt).getTime() - new Date(a.startedAt).getTime()
        valorB = new Date(b.endedAt).getTime() - new Date(b.startedAt).getTime()
        break
      default:
        return 0
    }
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Historial de Llamadas</h1>
              <p className="text-gray-600">Gestiona y analiza todas tus conversaciones</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-2xl font-bold text-gray-900">{llamadas.length}</p>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xl font-bold text-green-600">
                    {llamadas.filter((l) => l.analysis.successEvaluation === "true").length}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xl font-bold text-red-600">
                    {llamadas.filter((l) => l.analysis.successEvaluation === "false").length}
                  </p>
                </div>
                  ...
                  <p className="text-xl font-bold text-purple-600">
                    {Math.floor(
                      llamadas.reduce((sum, l) => {
                        const duracion = new Date(l.endedAt).getTime() - new Date(l.startedAt).getTime()
                        return sum + duracion
                      }, 0) / 60000,
                    )}
                    m
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="flex items-center gap-2">
                ...
                Estado: {filtroEstado === "todos" ? "Todos" : obtenerTextoEstado(filtroEstado)}
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent>
              <DropdownMenuItem onClick={() => setFiltroEstado("todos")}>Todos los estados</DropdownMenuItem>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <div className="text-sm text-gray-500">Haz clic en una fila para ver los detalles</div>
          </CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              ...
              <TableBody>
                {llamadasOrdenadas.map((llamada) => (
                  ...
                  >
                    <TableCell className="font-mono text-xs">{llamada.id.substring(0, 8)}...</TableCell>
                    <TableCell>
                      <div className="flex flex-col">
                        <span className="font-medium">{formatearFecha(llamada.startedAt)}</span>
                        <span className="text-xs text-gray-500">{llamada.type}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Badge className={obtenerColorEstado(llamada.status)} variant="outline">
                        {obtenerTextoEstado(llamada.status)}
                      </Badge>
                    </TableCell>
                    <TableCell className="max-w-[200px]">
                      <p className="text-sm text-gray-600 truncate">{obtenerResumenConversacion(llamada.transcript)}</p>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs text-gray-500 ml-1">
                          {llamada.messages.filter((m) => m.role !== "system").length} msgs
                        </span>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="font-medium">${llamada.cost.toFixed(4)}</TableCell>
                    <TableCell>
                      <Button
                        variant="ghost"
                        size="sm"
          ...
          {llamadasOrdenadas.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500">No se encontraron llamadas que coincidan con los filtros</p>
            </div>
          )}
        </CardContent>
      </Card>
