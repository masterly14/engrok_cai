...
export class ContactService {
  ...
  async getOrCreateContact(
    ...
    console.log(
      `[ContactService] Processing contact for agent: ${agentPhoneNumber}, client: ${clientPhoneNumber}`,
    );
    ...
    let contact = await this.findExistingContact(
      clientPhoneNumber,
      chatAgent.id,
    );
    ...
    if (!contact) {
      contact = await this.createNewContact(
        clientPhoneNumber,
        chatAgent.id,
        messageData,
      );
  ...
  private async createLeadIfNotExists(
    phoneNumber: string,
    chatAgent: any,
    messageData?: any,
  ) {
