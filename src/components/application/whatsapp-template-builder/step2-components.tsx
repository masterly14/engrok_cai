      ...
      case "BUTTONS":
        if (formData.category === "AUTHENTICATION") {
          newComponent = {
            type: "BUTTONS",
            buttons: [
              { type: "OTP", text: "Copy Code", otp_type: "COPY_CODE" } as any,
            ],
          };
        } else {
    ...
    <div className="space-y-6">
      ...
      {otherComponents.map((comp, idx) => {
        const originalIndex = components.findIndex(
          (c) => c.type === comp.type && c.text === comp.text,
        ); // Find original index based on more unique props if needed
      ...
      <div className="flex items-end gap-2 pt-4 border-t">
        <div className="flex-grow">
          ...
          <Select
            ...
            onValueChange={(val) => setComponentToAdd(val as any)}
          >
        ...
        <Button
          ...
          disabled={
            !componentToAdd || components.some((c) => c.type === componentToAdd)
          }
        >
      ...
      <p className="text-xs text-muted-foreground">
        El componente BODY es obligatorio. HEADER, FOOTER y BUTTONS son
        opcionales. Puedes reordenar los componentes arrastrándolos
