  ...
  const components = formData.components
    .map((comp) => {
      ...
      if (
        ...
        if (comp.example.header_text && comp.example.header_text.some((s: string) => s))
          cleanExample.header_text = comp.example.header_text
        if (comp.example.header_handle && comp.example.header_handle.some((s: string) => s))
          cleanExample.header_handle = comp.example.header_handle
        if (comp.example.body_text && comp.example.body_text.some((arr: string[]) => arr.some((s: string) => s)))
          cleanExample.body_text = comp.example.body_text
        if (comp.example.buttons && comp.example.buttons.some((arr: string[]) => arr.some((s: string) => s)))
          cleanExample.buttons = comp.example.buttons
        if (Object.keys(cleanExample).length > 0) newComp.example = cleanExample
      }
      ...
      if (comp.type === "BUTTONS" && comp.buttons && comp.buttons.length > 0) {
        newComp.buttons = comp.buttons.map((btn: ButtonComponent) => {
          ...
          if (btn.type === "URL" && btn.url) {
            apiBtn.url = btn.url
            if (btn.example && btn.example.length > 0 && btn.example.some((s: string) => s)) {
      ...
      } else if (comp.type === "BUTTONS" && (!comp.buttons || comp.buttons.length === 0)) {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-6">
      <div className="space-y-4">
        <h3 className="text-lg font-semibold">Previsualización de la Plantilla</h3>
      ...
      <div className="space-y-4">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Este es el JSON que se enviará a la API de WhatsApp.</CardDescription>
          </CardHeader>
          <CardContent>
            <Textarea value={jsonString} readOnly rows={20} className="text-xs font-mono bg-muted/30 max-h-[200px]" />
          </CardContent>
        </Card>
      </div>
    </div>
