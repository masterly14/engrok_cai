...
const generateEmptyExamples = (count: number): string[] =>
  Array(count).fill("");
...
export default function ComponentEditor({
  ...
  const [localComponent, setLocalComponent] =
    useState<TemplateComponent>(component);
  ...
  const handleFieldChange = (field: keyof TemplateComponent, value: any) => {
    ...
    if (
      field === "text" &&
      (updated.type === "BODY" ||
        (updated.type === "HEADER" && updated.format === "TEXT"))
    ) {
      ...
      if (updated.type === "BODY") {
        ...
        if (placeholderCount !== currentExamples.length) {
          updated.example = {
            ...updated.example,
            body_text: [generateEmptyExamples(placeholderCount)],
          };
        }
        ...
        if (placeholderCount !== currentExamples.length) {
          ...
          updated.example = {
            ...updated.example,
            header_text: [generateEmptyExamples(placeholderCount).join("")],
          };
        }
      }
    }
  ...
  const handleButtonChange = (
    ...
    if (field === "type" && value === "OTP") {
      updatedButtons[btnIndex].otp_type =
        updatedButtons[btnIndex].otp_type || "COPY_CODE";
    ...
    if (field === "url" && updatedButtons[btnIndex].type === "URL") {
      ...
      if (urlPlaceholderCount !== currentExample.length) {
        updatedButtons[btnIndex].example =
          generateEmptyExamples(urlPlaceholderCount);
      }
    }
  ...
  const handleButtonExampleChange = (
    btnIndex: number,
    exIndex: number,
    value: string,
  ) => {
  ...
  const removeUIButton = (btnIndex: number) => {
    const updatedButtons = (localComponent.buttons || []).filter(
  ...
  const renderHeaderFields = () => (
    <>
      <div className="mb-2">
        ...
        <Select
          value={localComponent.format}
          onValueChange={(val) =>
            handleFieldChange("format", val as HeaderFormat)
          }
        >
      ...
      {localComponent.format === "TEXT" && (
        <>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {(localComponent.text || "").length}/160 caracteres. Usa {"{{1}}"}{" "}
            para variables.
          </p>
          {(localComponent.example?.header_text || []).map(
            (ex: string, exIdx: number) => (
              <div key={`header-text-ex-${exIdx}`} className="mt-2">
                <Label htmlFor={`header-text-ex-${index}-${exIdx}`}>
                  Ejemplo para {"{{" + (exIdx + 1) + "}}"}
                </Label>
                <Input
                  id={`header-text-ex-${index}-${exIdx}`}
                  value={ex}
                  onChange={(e) =>
                    handleExampleChange("header_text", exIdx, e.target.value)
                  }
      ...
      {(localComponent.format === "IMAGE" ||
        ...
        <>
          <div className="mt-2">
            <Label htmlFor={`header-link-${index}`}>
              Link del Media (Imagen/Video/Documento)
            </Label>
            <Input
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">
              Proporciona un link público al archivo. Este se usará para el
  ...
  const renderBodyFields = () => {
      ...
      return (
        <div className="space-y-2">
          <label className="flex items-center space-x-2 text-sm">
            ...
            />
            <span>
              Incluir recomendación de seguridad ("Por tu seguridad, no
              compartas este código")
            </span>
          </label>
          <p className="text-xs text-muted-foreground">
            El texto del cuerpo lo genera WhatsApp automáticamente. Sólo puedes
            decidir si se añade la recomendación de seguridad.
          </p>
        </div>
      );
    }
    ...
    return (
      <>
        ...
        />
        <p className="text-xs text-muted-foreground mt-1">
          {(localComponent.text || "").length}/{maxLength} caracteres. Usa{" "}
          {"{{1}}"} para variables.
        </p>
        {(localComponent.example?.body_text?.[0] || []).map(
          (ex: string, exIdx: number) => (
            <div key={`body-ex-${exIdx}`} className="mt-2">
              <Label htmlFor={`body-ex-${index}-${exIdx}`}>
                Ejemplo para {`{{${exIdx + 1}}}`}
              </Label>
              <Input
                ...
                value={ex}
                onChange={(e) =>
                  handleExampleChange("body_text", exIdx, e.target.value)
                }
                placeholder={`Valor para {{${exIdx + 1}}}`}
              />
            </div>
    ...
    if (category === "AUTHENTICATION") {
      return (
        <div className="space-y-1">
          <Label htmlFor={`exp-min-${index}`}>
            Minutos hasta que caduque el código
          </Label>
          <Input
            id={`exp-min-${index}`}
            type="number"
          ...
          />
          <p className="text-xs text-muted-foreground">
            Entre 1 y 90. WhatsApp mostrará "Este código caduca en X minutos".
          </p>
        </div>
      );
    }
  ...
  const renderButtonsFields = () => (
    <div className="space-y-3">
      {(localComponent.buttons || []).map(
        (btn: ButtonComponent, btnIdx: number) => (
          <Card key={btnIdx} className="p-3 bg-muted/30">
            ...
            <Input
              ...
              value={btn.text}
              onChange={(e) =>
                handleButtonChange(btnIdx, "text", e.target.value)
              }
              maxLength={btn.type === "QUICK_REPLY" ? MAX_QUICK_REPLY_TEXT : 20} // Display text for URL/Phone is shorter
              className="mt-2"
            />
            <p className="text-xs text-muted-foreground mt-1">
              Máx: {btn.type === "QUICK_REPLY" ? MAX_QUICK_REPLY_TEXT : 20}{" "}
              caracteres.
            </p>
            ...
            {btn.type === "URL" && category !== "AUTHENTICATION" && (
              <>
                <Input
                  ...
                  value={btn.url || ""}
                  onChange={(e) =>
                    handleButtonChange(btnIdx, "url", e.target.value)
                  }
                ...
                {(btn.example || []).map((exVal: string, exIdx: number) => (
                  <div key={`btn-url-ex-${btnIdx}-${exIdx}`} className="mt-1">
                    <Label
                      htmlFor={`btn-url-ex-${btnIdx}-${exIdx}`}
                      className="text-xs"
                    >
                    ...
                    <Input
                      ...
                      value={exVal}
                      onChange={(e) =>
                        handleButtonExampleChange(btnIdx, exIdx, e.target.value)
                      }
                      placeholder={`Valor para {{${exIdx + 1}}}`}
                    />
                  </div>
            ...
            {btn.type === "PHONE_NUMBER" && category !== "AUTHENTICATION" && (
              <Input
                ...
                onChange={(e) =>
                  handleButtonChange(btnIdx, "phone_number", e.target.value)
                }
                className="mt-2"
              />
            )}
            {btn.type === "OTP" && (
              <div className="space-y-2 mt-2">
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    handleButtonChange(btnIdx, "otp_type", val)
                  }
                >
      ...
      {(localComponent.buttons?.length || 0) < 3 &&
        !(
          category === "AUTHENTICATION" &&
          (localComponent.buttons?.length || 0) >= 1
        ) && (
          <Button
            variant="outline"
            onClick={addUIButton}
      ...
      {(localComponent.buttons?.length || 0) > 0 &&
        ...
        localComponent.buttons?.some(
          ...
          <div className="mt-2 p-2 text-xs bg-yellow-100 text-yellow-700 rounded flex items-center">
            ...
            Mezclar botones de Respuesta Rápida con URL/Teléfono puede no
            mostrarse en WhatsApp Desktop.
          </div>
        )}
      {category === "AUTHENTICATION" &&
        !(localComponent.buttons || []).some(
          (b: ButtonComponent) => b.type === "OTP",
        ) && (
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="cursor-grab mr-2 focus-visible:ring-0"
                >
