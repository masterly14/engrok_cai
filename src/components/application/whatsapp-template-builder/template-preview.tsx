...
import { ButtonComponent, TemplateComponent } from "@/actions/whatsapp/templates"
...
const fillPlaceholders = (text = "", examples: string[] = []): string => {
  ...
  examples.forEach((example, i) => {
    result = result.replace(new RegExp(`\\{\\{${i + 1}\\}\\}`, "g"), example || `{{${i + 1}}}`)
  })
  return result
}
...
const IosPreview: React.FC<{ formData: TemplateFormData }> = ({ formData }) => {
  const headerComp = formData.components.find((c: TemplateComponent) => c.type === "HEADER")
  const bodyComp = formData.components.find((c: TemplateComponent) => c.type === "BODY")! // Body is mandatory
  const footerComp = formData.components.find((c: TemplateComponent) => c.type === "FOOTER")
  const buttonsComp = formData.components.find((c: TemplateComponent) => c.type === "BUTTONS")
  ...
  const filledHeaderText = headerComp?.format === "TEXT" ? fillPlaceholders(headerComp.text, headerTextExamples) : ""
  ...
  return (
    <div className="bg-[#E5DDD5] p-4 rounded-lg w-[320px] shadow-md font-sans">
      <div className="bg-white rounded-lg p-2.5 text-[14.2px] leading-snug shadow-sm min-h-[60px]">
        ...
        {headerComp && (
          <div className="mb-1">
            {headerComp.format === "TEXT" && filledHeaderText && (
              <p className="font-semibold text-gray-800 break-words">{filledHeaderText}</p>
            )}
            ...
            {headerComp.format === "DOCUMENT" && (
              <div className="bg-gray-200 p-2 rounded-md flex items-center text-gray-600">
                <MessageSquareText className="h-6 w-6 mr-2 text-gray-500" /> Documento.pdf
              </div>
            )}
          </div>
        )}
        ...
        {/* Body */}
        <p className="text-gray-800 whitespace-pre-wrap break-words">{filledBody || "Cuerpo del mensaje..."}</p>

        {/* Footer */}
        {footerComp && footerComp.text && <p className="text-xs text-gray-500 mt-1 break-words">{footerComp.text}</p>}
      </div>
...
const AndroidPreview: React.FC<{ formData: TemplateFormData }> = ({ formData }) => {
  const headerComp = formData.components.find((c: TemplateComponent) => c.type === "HEADER")
  const bodyComp = formData.components.find((c: TemplateComponent) => c.type === "BODY")!
  const footerComp = formData.components.find((c: TemplateComponent) => c.type === "FOOTER")
  const buttonsComp = formData.components.find((c: TemplateComponent) => c.type === "BUTTONS")
    ...
    <div className="bg-[#ECE5DD] p-3 rounded-lg w-[320px] shadow-lg font-roboto">
      ...
      <div className="bg-white rounded-lg p-2 shadow-[0_1px_0.5px_rgba(0,0,0,0.13)] text-[15px] leading-relaxed min-h-[60px]">
        ...
        {headerComp && (
          <div className="mb-1">
            {headerComp.format === "TEXT" && filledHeaderText && (
              <p className="font-bold text-black break-words">{filledHeaderText}</p> // Slightly bolder
            )}
            {headerComp.format === "IMAGE" && (
            ...
            {headerComp.format === "DOCUMENT" && (
              <div className="bg-gray-100 p-2 rounded flex items-center text-gray-700 border border-gray-200">
                <MessageSquareText className="h-5 w-5 mr-1.5 text-gray-500" /> Documento.pdf
              </div>
            )}
          </div>
        )}
        <p className="text-black whitespace-pre-wrap break-words">{filledBody || "Cuerpo del mensaje..."}</p>
        {footerComp && footerComp.text && (
          <p className="text-xs text-[#667781] mt-0.5 break-words">{footerComp.text}</p> // Different muted color
        )}
      </div>
      {buttonsComp && buttonsComp.buttons && buttonsComp.buttons.length > 0 && (
...
const DesktopPreview: React.FC<{ formData: TemplateFormData }> = ({ formData }) => {
  const headerComp = formData.components.find((c: TemplateComponent) => c.type === "HEADER")
  const bodyComp = formData.components.find((c: TemplateComponent) => c.type === "BODY")!
  const footerComp = formData.components.find((c: TemplateComponent) => c.type === "FOOTER")
  const buttonsComp = formData.components.find((c: TemplateComponent) => c.type === "BUTTONS")
  ...
  const hasQuickReply = buttonsComp?.buttons?.some((b: ButtonComponent) => b.type === "QUICK_REPLY")
  const hasOtherButtons = buttonsComp?.buttons?.some((b: ButtonComponent) => b.type === "URL" || b.type === "PHONE_NUMBER")
    ...
    <div className="bg-gray-100 p-4 rounded-lg w-[450px] shadow-md font-sans border border-gray-300">
      <div className="bg-white rounded-md p-3 text-sm leading-normal shadow-sm min-h-[70px]">
        ...
        {headerComp && (
          <div className="mb-1.5">
            {headerComp.format === "TEXT" && filledHeaderText && (
              <p className="font-bold text-gray-900 break-words">{filledHeaderText}</p>
            )}
            {headerComp.format === "IMAGE" && (
            ...
            {headerComp.format === "DOCUMENT" && (
              <div className="bg-gray-200 p-2.5 rounded-md flex items-center text-gray-700 border">
                <MessageSquareText className="h-6 w-6 mr-2 text-gray-600" /> Documento_grande_final.pdf
              </div>
            )}
          </div>
        )}
        ...
        {footerComp && footerComp.text && <p className="text-xs text-gray-500 mt-1.5 break-words">{footerComp.text}</p>}
      </div>

      {buttonsComp && buttonsComp.buttons && buttonsComp.buttons.length > 0 && !showDesktopButtonsWarning && (
          ...
          {buttonsComp.buttons.map((btn: ButtonComponent, idx: number) => (
            ...
            >
              ...
              {btn.type === "URL" && btn.url && (
                <span className="text-xs block text-gray-500">({fillUrlPlaceholders(btn.url, btn.example)})</span>
              )}
            </div>
          ))}
        </div>
      ...
      {showDesktopButtonsWarning && (
        <div className="mt-2 p-2 text-xs bg-yellow-100 text-yellow-700 rounded text-center">
          Los botones de Respuesta Rápida pueden no mostrarse junto con botones URL/Teléfono en Escritorio.
        </div>
      )}
...
export default function TemplatePreviewDisplay({ formData }: TemplatePreviewProps) {
  return (
