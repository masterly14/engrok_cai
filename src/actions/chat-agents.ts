...
export const getAvailableWorkflows = async (agentId?: string) => {
  ...
  return db.chatWorkflow.findMany({
    where: {
      userId: user.data.id,
      OR: [{ agentId: null }, { agentId: agentId ?? undefined }],
    },
    orderBy: {
      name: "asc",
    },
...
export const updateChatAgent = async (
  ...
  return db.$transaction(async (prisma) => {
    ...
    const currentWorkflow = currentAgentState.workflows.find(
      (w) => w.agentId === agentId,
    );
...
export const saveUserAsset = async (params: {
  name: string;
  type: string;
  url: string;
}) => {
