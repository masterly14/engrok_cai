...
const sendToVapiAPI = async (vapiPayload: any) => {
  try {
    ...
    if (!process.env.VAPI_API_KEY) {
      throw new Error(
        "VAPI_API_KEY no está configurada en las variables de entorno",
      );
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Vapi API error: ${errorData.message || response.statusText} (Status: ${response.status})`,
      );
    }
...
const getVapiWorkflow = async (vapiWorkflowId: string) => {
  try {
    ...
    const response = await fetch(
      `https://api.vapi.ai/workflow/${vapiWorkflowId}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${process.env.VAPI_API_KEY}`,
...
export const createWorkflow = async (data: any) => {
  ...
  try {
    ...
    return {
      status: 200,
      workflow: workflow,
      vapiResult: data.vapiPayload
        ? await sendToVapiAPI(data.vapiPayload)
        : null,
    };
  } catch (error) {
    return {
...
export const syncWorkflowWithVapi = async (workflowId: string) => {
  ...
  try {
    ...
    if (workflow.vapiWorkflowId) {
      ...
      const response = await fetch(
        `https://api.vapi.ai/workflow/${workflow.vapiWorkflowId}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${process.env.VAPI_API_KEY}`,
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to update Vapi workflow: ${response.statusText}`,
        );
      }
