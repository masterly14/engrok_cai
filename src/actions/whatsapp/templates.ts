...
export const createMessageTemplate = async (params: CreateTemplateParams) => {
  ...
  const apiComponents = components.map((comp) => {
    ...
    if (category === "AUTHENTICATION") {
      if (
        comp.type === "BODY" &&
        comp.add_security_recommendation !== undefined
      ) {
        newComp.add_security_recommendation = comp.add_security_recommendation;
      }
      if (
        comp.type === "FOOTER" &&
        comp.code_expiration_minutes !== undefined
      ) {
  ...
  try {
    ...
    if (!agentId) {
      throw new Error(
        "[createMessageTemplate] agentId is required but was not provided",
      );
    }
...
export const updateMessageTemplate = async (params: UpdateTemplateParams) => {
  ...
  if (!category && !components) {
    return {
      success: false,
      error: "Debes enviar 'category' o 'components' para actualizar.",
    } as const;
  }
  ...
  if (components) {
    payload.components = components.map((comp) => {
      ...
      if (comp.type === "HEADER" && comp.format === "TEXT") {
        // WhatsApp requiere que example.header_text esté siempre presente si el HEADER es de texto
        const headerTextExample = comp.example?.header_text ?? [
          comp.text ?? "",
        ];
        newComp.example = {
          ...(comp.example || {}),
      ...
      if (category === "AUTHENTICATION") {
        if (
          ...
          newComp.add_security_recommendation =
            comp.add_security_recommendation;
        }
        if (
          comp.type === "FOOTER" &&
          comp.code_expiration_minutes !== undefined
